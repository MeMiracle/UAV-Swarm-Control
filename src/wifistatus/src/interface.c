 ...
 */
static int get_iface_name(struct nl_msg *msg, void *arg) {
	...
	{
		strncpy(__wifi.ifname, nla_get_string(tb_msg[NL80211_ATTR_IFNAME]),
				strlen(nla_get_string(tb_msg[NL80211_ATTR_IFNAME])));
	}

	if (tb_msg[NL80211_ATTR_WIPHY_FREQ])
		__wifi.freq = nla_get_u32(tb_msg[NL80211_ATTR_WIPHY_FREQ]);
			   ...
			   enum id_input id)
{
	...
	register_handler(get_iface_name, &dev_dump_wiphy);
	return 0;
}
