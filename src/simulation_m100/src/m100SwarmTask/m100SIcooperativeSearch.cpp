//遗留问题，中断搜索后第一次发点targetsize为0，第二次正常
#include <stdio.h>
#include <stdlib.h>
#include <cmath>
#include <string>
...
#define STEP_TIME 0.5//一次步进时间
#define C_PI (double)3.141592653589793
#define C_EARTH (double)6378137.0
#define MAPXLENGTH 15//图长
#define MAPYLENGTH 20//图长
#define DECAYFACTOR 0.995//衰减因子
#define MAP_FACTOR 1//图长除以边长
#define STEPCOUNT 400//运动总步长
#define MAP_BUFFER 25//交换地图长度
#define DISTURBANCE 0.03//转向扰动概率
#define RE_COVER 0.3//重覆盖阈值

ofstream recordROSInfo;//记录命令行输出信息
ofstream recordLocalMap;//记录接收的局部信息素图
...
enum direction {
	left, left_up, up, right_up, right, right_down, down, left_down, here, rise, descend
};
...
public:
	...
	void moveTowardDir(direction adjDir, int moveCtrlMode)
	{
		...
		//recordROSInfo<<"uav%d move to %d"<<ID<<adjDir;
...
float _map[MAPXLENGTH][MAPYLENGTH];//信息素图

int validitycheckx(int i) {
	if (i > MAPXLENGTH - 1)
		i = int(MAPXLENGTH - 1);
	else if (i < 0)
		i = 0;
...
void updatemap() {//无人系统移动后更新信息素图
	...
	int mapi = validitycheckx(int(uav.getPosition().x * MAP_FACTOR));
	int mapj = validitychecky(int(uav.getPosition().y * MAP_FACTOR));
	_map[mapi][mapj] = 1;
}
		...
		for (int jj = j + 2; jj >= j - 2; jj--)
			for (int ii = i - 2; ii <= i + 2; ii++)
			{
				if (mapbuffer[index]>0)
					if (mapbuffer[index] > _map[ii][jj])
						_map[ii][jj] = mapbuffer[index];
				index++;
			}
	}
	if (MAP_BUFFER == 49)
...
float coverrate() {
	...
	return cover / (MAPXLENGTH*MAPYLENGTH);
}
	...
	if (strcmp(result.component.c_str(), component.c_str()) == 0)
	{
		if (strcmp(result.command.c_str(), move_cmd.c_str()) == 0)
		{
			...
			if (cmd_status == false)
			{
				...
				recordROSInfo<<"move_refused!"<<endl;
			}
		}
	}
}
...
vector<float> getMapBuffer(vehicle us)
{
	...
	if (MAP_BUFFER == 9)
	{
		...
		i = seedi + 1;
		if (i > MAPXLENGTH - 1 || j > MAPYLENGTH - 1)
			mapbuffer[2] = 0;
		else
	...
	if (MAP_BUFFER == 25)
	{
		...
		int seedj = int(us.getPosition().y*MAP_FACTOR);
		int index = 0;
		for (int j = seedj + 2; j >= seedj - 2; j--)
			for (int i = seedi - 2; i <= seedi + 2; i++)
			{
				if (i<0 || j<0 || i>MAPXLENGTH - 1 || j>MAPYLENGTH - 1)
					mapbuffer[index] = 0;
				else
					mapbuffer[index] = getmapV(i, j);
				index++;
			}
	}
	if (MAP_BUFFER == 49)
	{
		...
		int seedj = int(us.getPosition().y*MAP_FACTOR);
		int index = 0;
		for (int j = seedj + 3; j >= seedj - 3; j--)
			for (int i = seedi - 3; i <= seedi + 3; i++)
			{
				if (i<0 || j<0 || i>MAPXLENGTH - 1 || j>MAPYLENGTH - 1)
					mapbuffer[index] = 0;
				else
...
void updateMapBuffer(std::vector<mapbuffer::MapBuffer> &swarmMap)
{
	...
	for (int i = 0; i < swarmMap.size(); i++)//swarmMap
	{
		...
		{
			recordLocalMap <<endl<< "Length "<<swarmMap[i].length() << " seedi "<<swarmMap[i].seedi()<<" seedj "<<swarmMap[i].seedj();
			vector<float> mb;
			for(int j=0;j<swarmMap[i].mb_item_size();j++)
			{
				mb.push_back(swarmMap[i].mb_item(j));
				recordLocalMap << " "<< swarmMap[i].mb_item(j);
			}
...
void subSwarmWaypointCallback(simulation_m100::PubSubUavMsg target_msg)//实时更新目标航点到达状态
{
				...
				for (int j = 0; j<targetBuf.size(); j++)
				{
						...
						abs(waypoint.z() - targetBuf[j].point.z) <= 0.01)
					{
						if (targetBuf[j].markerID == uav.getID())//该点是自己的目标点，则接受修改（有其他无人系统先到达了某个目标点）
						{
							...
							recordROSInfo<<"notify:someone(uav%d) reached the target:%d"<< -waypoint.mark()<< j + 1<<endl;
							break;
						}
					}
				}
...
void updateTarget(vector<Target> &target)
{
	...
	//ROS_INFO("target.size():%u",target.size());
...
void tryToDeclareTarget(vector<Target> target, int targetIndex)
{
	...
	if (reachFirst)//本机先到达目标点，则广播一次该位置已经到达
	{
		...
		recordROSInfo<< "I(uav%d) reached target:%d\n"<< uav.getID()<< targetIndex + 1<<endl;
	...
	else//其他无人机先到达了目标点
	{
		...
		recordROSInfo <<"others(uav%d) a little earlier reached target:%d"<< -target[targetIndex].markerID<< targetIndex + 1<<endl;
	}
}
...
*/
{
	...
	while (flg && true == cmd_status)
	{
		...
		if (!barrierflg)
		{
			...
			for (auto it = swarmPosition.begin(); it != swarmPosition.end(); it++)
			{
				if (abs(attemptPosition - (*it)) < 2*VEHICLE_DIAMETER)
		...
		if (swarmflg == 1)
		{
			ROS_INFO("%d send:%d",count,moveDirection);
			recordROSInfo<<count<< "send:%d"<< moveDirection<<endl;
...
{
	if (abs(p1.z - p2.z) > 0.5*VEHICLE_DIAMETER)
	{
		return false;
	}
...
*/
{
	if (targetPosition.z - nowPosition.z >= 0.5*VEHICLE_DIAMETER)
	{
...
*/
{
	...
	mapi = validitycheckx(mapi);
	mapj = validitychecky(mapj);
	int i = mapi, j = mapj;//一定要整型

	direction moveDirection = us.getoldD();//直走
	if (us.getoldD() == here) {
		...
		while (bf1 && true == cmd_status) {
			...
			else if (i > (MAPXLENGTH - 1) && j < 0)
			{
				moveDirection = direction(1);
				bf1 = true;
			}
			else if (i < 0 && j >(MAPYLENGTH - 1))
			{
				moveDirection = direction(5);
				bf1 = true;
			}
			else if (i >(MAPXLENGTH - 1))
			{
	...
	while (flg && true == cmd_status)//候选运动方向前进的网格是否未搜索
	{
		...
		if (i >= 0 && i<MAPXLENGTH && j >= 0 && j<MAPYLENGTH)
		{
			if ((flg == 1) && (getmapV((int)i, (int)j) >= RE_COVER)) {
				float r = (rand() / (RAND_MAX + 1.0));
				if (r > (1 - 10 * DISTURBANCE))
				{
					moveDirection = direction((us.getoldD() + 1) % 8);//有一定概率右转
					recordROSInfo << "RE cover turn right" << endl;
				}
				else if (r < 10 * DISTURBANCE)
				{
					moveDirection = direction((us.getoldD() + 7) % 8);//有一定概率左转
					recordROSInfo << "RE cover turn left" << endl;
				}
...
void sendMapBuffer(vector<float> &MB,int seedi,int seedj,int mapbufferlength,int usid)
{
	...
	simulation_m100::PubSubUavMsg ps_msg;
...
void* Cooperative_search(void* args)
{
	...
	ofstream recordMap;
	ofstream recordTarget;

	char posFileName[] = "simulation_m100/log/recordPosition";
	string pos_file(getDirectory() + posFileName + uavName.c_str() + ".txt");

	char dirFileName[] = "simulation_m100/log/recordDirection";
	string dir_file(getDirectory() + dirFileName + uavName.c_str() + ".txt");

	char mapFileName[] = "simulation_m100/log/recordMap";
	string map_file(getDirectory() + mapFileName + uavName.c_str() + ".txt");

	char tarFileName[] = "simulation_m100/log/recordTarget";
	string tar_file(getDirectory() + tarFileName + uavName.c_str() + ".txt");
	...
	recordTarget.open(tar_file.c_str(), ios::app);

	bool oneflag = true;//一次实验标记
	while (ros::ok())
	{
		while (false == cmd_status || uav.getPosition().z <= 0.5)//任务被中止或无人机在地面时
		{
		...
		{
			oneflag = false;
			ROS_INFO("uav.height=%f", uav.getPosition().z);
			recordROSInfo<<"uav.height="<<uav.getPosition().z<<endl;
			ROS_INFO("cooperative search  is on standby");
			recordROSInfo<<"cooperative search  is on standby"<<endl;
			recordPosition << ros::Time::now() << endl<<endl;
			recordDirection << ros::Time::now() << endl<<endl;
			recordMap << ros::Time::now() << endl<<endl;
			recordLocalMap << ros::Time::now() << endl << endl;
			recordTarget << ros::Time::now() << endl<<endl;
		...
		while (false == ifFinishAll && true == cmd_status)//监听条件：所有目标点是否完成&任务是否被中断
		{
			...
			if (target.size() > 0)
			{
				for (int i = 0; i < target.size(); i++)
				{
					if (target[i].markerID == uav.getID())
					{
						...
						targetIndex = i;
						ROS_INFO("targetPoint:%d,%f,%f,%f", targetIndex, targetPoint.x, targetPoint.y, targetPoint.z);
						recordROSInfo << "targetPoint:" << targetIndex << " " << targetPoint.x << " " << targetPoint.y << " " << targetPoint.z <<" "<< target[i].markerID <<endl;

						recordTarget << targetIndex << " " <<fixed<<setprecision(6)<< (double)targetPoint.x << " " << (double)targetPoint.y << " " << (double)targetPoint.z << endl;
						//ROS_INFO("lastTargetIndex:%d",lastTargetIndex);
						//ros::Duration(1).sleep();				
			...
			if (count == STEPCOUNT)//已到达指定运动步长，则停止 
			{
				...
				recordROSInfo<<"all steps finished!"<<endl;
				oneflag = true;
				ifFinishAll = true;
				break;
			}
			...
			if (true == new_target)
			{
				...
				ROS_INFO("targetIndex %d...", targetIndex);
				ROS_INFO("targetPoint:%f,%f,%f", targetPoint.x, targetPoint.y, targetPoint.z);

				new_target = false;
				//targetflg = false;是否需要，存疑，等待验证//和标记1联动
			}
			...
			if (!targetflg) {
				if (!isApproimate(targetPoint, myPosition)) {
					...
					ROS_INFO("move to targetPoint:%f,%f,%f", targetPoint.x, targetPoint.y, targetPoint.z);
                    ROS_INFO("myPosition:%f,%f,%f", myPosition.x, myPosition.y, myPosition.z);
					recordROSInfo << "move to targetPoint:" << targetPoint.x << " " << targetPoint.y << " " << targetPoint.z <<endl;
					recordROSInfo << "myPosition:" << myPosition.x << " " << myPosition.y <<" "<< myPosition.z <<endl;
				}
				else {
					//更新目标位置序列
					updateTarget(target);
			...
			if (targetflg) {//不需要抵达目标地，执行搜索
				...
				recordROSInfo<<"Searching..myPosition:"<< myPosition.x<<" "<<myPosition.y << " "<<myPosition.z<<endl;
				ROS_INFO("Searching.. myPosition:%f,%f,%f", myPosition.x, myPosition.y, myPosition.z);

				if (target[targetIndex].markerID == uav.getID())//有新的目标点到来/////////和标记1联动
				{
					targetflg = false;
				}
			}
			...
			sendMapBuffer(MB,seedi,seedj,mapbufferlength,usid);//发送自己的mapbuffer到其他无人系统，参数列表可修改，内容为上面5变量
			updateMapBuffer(swarmMap);//根据其他无人系统位置更新信息素图


			uav.moveTowardDir(moveDirection, moveCtrlMode);

			ros::Time start_time = ros::Time::now();
			while (ros::ok() && true == cmd_status)//让无人机运动一定的时间
			{
				ros::Duration(0.1).sleep();
			...
			recordPosition << myPosition.x << " " << myPosition.y << " " << myPosition.z << " "<<coverrate()<<endl;
			count++;
		}
		...
		ros::Duration(0.1).sleep();//完成一次任务后，在接到下一个任务前保持间歇休眠以出让cpu
	}
...
int main(int argc, char** argv)
{
	...
	ros::Subscriber other_mb_sub = nh.subscribe(string(uavName + "/other_uav_mapbuffer").c_str(), 10, subOtherMBCallback);//局部信息素图
	...
	mapbuffer_pub = nh.advertise<simulation_m100::PubSubUavMsg>(string(uavName + "/my_mapbuffer").c_str(),10);//局部信息素图通告

	for (int i = 0; i<MAPXLENGTH; i++)//初始化信息素图
	for (int j = 0; j < MAPYLENGTH; j++)
	{
		_map[i][j] = 0.05;
	}
	string info_file=getDirectory() + "simulation_m100/log/recordROSInfo.txt";
	recordROSInfo.open(info_file.c_str(),ios::app);
	char localmapFileName[] = "simulation_m100/log/recordLocalMap";
	string localmap_file(getDirectory() + localmapFileName + uavName.c_str() + ".txt");
	recordLocalMap.open(localmap_file.c_str(), ios::app);
