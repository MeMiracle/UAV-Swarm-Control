...
float coverrate(){
	...
	return cover/(MAPLENGTH*MAPLENGTH);
}
...
void subSwarmWaypointCallback(simulation_m100::PubSubUavMsg target_msg)//实时更新目标航点到达状态
{
	...
	ROS_INFO("received %d waypoints!",target_waypoints.point_size());
...
{
	pthread_mutex_lock(&target_mutex);
	//ROS_INFO("targetBuf.size()=%ld",targetBuf.size());
	target = targetBuf;
	//ROS_INFO("target.size()=%ld",target.size());
	pthread_mutex_unlock(&target_mutex);
...
void* Cooperative_search(void* args)
{
	while (ros::ok())
	{
		ROS_INFO("targetBuf.size=%ld uav.height=%f", targetBuf.size(), uav.getPosition().z);
		while (true == cmd_status && uav.getPosition().z <= 0)
		{
		...
		while (false == ifFinishAll && true == cmd_status)//监听条件：所有目标点是否完成&任务是否被中断
		{
			...
			if (target.size() > 0)
			{
				...
				{
					//ROS_INFO("target.markerID=%d",target[i].markerID);
...
int main(int argc, char** argv)
{
	...
	mapbuffer_pub = nh.advertise<simulation_m100::PubSubUavMsg>(string(uavName + "/my_mapbuffer").c_str(),10);
