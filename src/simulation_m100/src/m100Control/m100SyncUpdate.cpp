...
string component = "m100SyncUpdate";
string move_cmd = "move_cmd";
int sync_update_priority = 90;
...
void syncUpdate_task_callback(const simulation_m100::PubSubUavMsg move_cmd_msg);//订阅并分析控制指令
void subCmdResultCallback(simulation_m100::CmdResult result);

pthread_t syncUpdateThread;
void* syncFly();
void estimateOtherUavData();
...
{
    char pos_log[] = "simulation_m100/log/swarm_pos_log_";
	...
	char vel_log[] = "simulation_m100/log/swarm_vel_log_";
...
int main(int argc, char *argv[])
{
    ...
    ros::init(argc, argv, "m100SyncUpdate");
    ros::NodeHandle nh;

	ROS_INFO("m100SyncUpdate_node started.");
    ...
    ros::Subscriber syncUpdate_task_sub = nh.subscribe(string(uav_name + "/syncfly_task").c_str(),10,&syncUpdate_task_callback);
	...
	//pthread_create(&syncUpdateThread,NULL,syncFly,NULL);
    //ros::Duration(0.5).sleep();//等待线程启动
    //ROS_INFO("syncUpdateThread is started.");
...
void subOtherUavFdDataCallback(simulation_m100::PubSubUavMsg other_uav_fd_data_msg)//实时更新其他无人系统状态数据
{
	...
	if(ros::Time::now().toSec()-timeToUpdate >= delay)
	{
		...
		pos_frame.seq_num =  other_uav_fd_data_msg.seq_num;
		pos_frame.time_stamp = other_uav_fd_data_msg.time_stamp;

		vel_frame.seq_num =  other_uav_fd_data_msg.seq_num;
		vel_frame.time_stamp = other_uav_fd_data_msg.time_stamp;
		...
		pos_frame.point = other_uav_pos;
		vel_frame.point = other_uav_vel;
...
int XorY = 0;
double velocity = 1;
void syncUpdate_task_callback(const simulation_m100::PubSubUavMsg syncUpdate_task_msg)//
{
	myTaskMessage::TaskMessage task_waypoints;
    task_waypoints.ParseFromString(syncUpdate_task_msg.playload);
...
void* syncFly()//持续发布指令
{
	...
	//{
		...
		else
		{
            ...
            playload_msg.cmd_priority = sync_update_priority;
