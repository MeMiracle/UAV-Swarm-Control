...
#include <simulation_m100/ObstacleMsg.h>
...
vector<cloudType> remove_invalid_data(sensor_msgs::LaserScan& scan)	// 输入点云，返回包含有效点云的数组
{
	...
	for (int i=0;i<count_total;i++)
	{
		...
		{   
			...
			temp.x = scan.ranges[i]*cos(DEG2RAD(degree));
			temp.y = scan.ranges[i]*sin(DEG2RAD(degree));
			temp.z = 0;	// warning
			temp.num = i;
...
vector<point> DBSCAN(vector<cloudType>& cloudTemp, ros::Publisher *pub) // 改进 DBSCAN 聚类，返回
{
    ...
    if (clusterIndexNum[1]>clusterMemberThreshold)
    {
        ...
        {
            tempPoint1.x = min_radius*cos(DEG2RAD(first_degree));
            tempPoint1.y = min_radius*sin(DEG2RAD(first_degree));
            tempPoint1.z = 0;
            tempPoint2.x = min_radius*cos(DEG2RAD(second_degree));
            tempPoint2.y = min_radius*sin(DEG2RAD(second_degree));
            tempPoint2.z = 0;
...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan, ros::Publisher *pub)
{
    ...
    //ROS_INFO("corecloud size: %d", corecloud.size());
    ...
    #if VIEWER_SHOW
    {
        ...
        */
        ...
        {
            cloud_obstacle->points[2*i].x = obstacle.center[i].x*cos(DEG2RAD( obstacle.center[i].z));
            cloud_obstacle->points[2*i].y = obstacle.center[i].x*sin(DEG2RAD( obstacle.center[i].z));
            cloud_obstacle->points[2*i].z = 0;

            cloud_obstacle->points[2*i+1].x = obstacle.center[i].x*cos(DEG2RAD( obstacle.center[i].y));
            cloud_obstacle->points[2*i+1].y = obstacle.center[i].x*sin(DEG2RAD( obstacle.center[i].y));
    ...
    if (logging)
    {
        char lidar_fileName[] = "simulation_m100/log/lidar_log.txt";
        ...
        if(lidar_str)
        {
            int tempCount = scan->ranges.size();

            pcl::PointCloud<pcl::PointXYZ>::Ptr log_cloud (new pcl::PointCloud<pcl::PointXYZ>);
            log_cloud->height = 1;
            log_cloud->width  = tempCount;
            log_cloud->points.resize(log_cloud->width * log_cloud->height);
            
            for(int i = 0; i < tempCount; i++) 
            {
                float degree = RAD2DEG(scan->angle_min + scan->angle_increment * i);  
                if(scan->ranges[i] && (_isinf(scan->ranges[i])==0)){
                    log_cloud->points[i].x = scan->ranges[i]*cos(DEG2RAD(degree));
                    log_cloud->points[i].y = scan->ranges[i]*sin(DEG2RAD(degree));
                    log_cloud->points[i].z = 0;
                }
                else
                {
                    log_cloud->points[i].x = 0;
                    log_cloud->points[i].y = 0;
                    log_cloud->points[i].z = 0;
                }
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher obstacle_pub = n.advertise<simulation_m100::ObstacleMsg>(string(uav_name + "/obstacle").c_str(), 1000);
    ros::Subscriber sub = n.subscribe<sensor_msgs::LaserScan>(string(uav_name + "/scan").c_str(), 1000, boost::bind(&scanCallback, _1, &obstacle_pub));
