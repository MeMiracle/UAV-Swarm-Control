...
#define Formation_expansion_factor 1.5
...
void uav_zyl::moveTowardDir(direction adjDir, int moveCtrlMode)
{
...
//    cout<<"wo de tian a "<<res.x<<res.y<<res.z<<endl;
    ...
    for(int i = 1;i<other_uav_count;i++)
    {
        if(i == _uav.getUavID())
            continue;
        x += swarmlocation[i].x;
...
void onestep(uav_zyl& thecar,Point3D movePoint,fstream& recordcar)
{
    struct timeval tv;
    struct timezone tz;
    ...
    tempDir = calculateDirection(movePoint,thecar.getPosition(),Radius);
    tempDir2 = tempDir;
    ...
    thecar.moveTowardDir(moveDir,moveCtrlMode);
    gettimeofday(&tv, &tz);
    p = localtime(&tv.tv_sec);

    recordcar<<movecount++<<","<<p->tm_hour<<","<< p->tm_min<<","<< p->tm_sec<<","
             << tv.tv_usec<<"," <<uav.x_speed<<","<<uav.y_speed<<","<<thecar.z_speed<<","<<thecar.getPosition().x
             <<","<<thecar.getPosition().y<<","<<thecar.getPosition().z<<","<<movePoint.x - thecar.getPosition().x<<","
             <<movePoint.y - thecar.getPosition().y<<","
             <<movePoint.z - thecar.getPosition().z<<myBehavior<<endl;
    // cout<<movecount<<","<<p->tm_hour<<","<< p->tm_min<<","<< p->tm_sec<<","
    //     << tv.tv_usec<<"," <<uav.x_speed<<","<<uav.y_speed<<","<<uav.getPosition().x
    //     <<","<<uav.getPosition().y<<movePoint.x - thecar.getPosition().x<<","
    //     <<movePoint.y - thecar.getPosition().y<<","
    //     <<movePoint.z - thecar.getPosition().z<<endl;
    //此处为需要记录的信息
 ...
 */
{
    ...
    switch(other_uav_count)
    {
        case 0:
        {
            while(!(*thisflag))
            {
                ...
                {
                    direction tempDir;
                    onestep(uav,myFormationTarget.point,recordFormation3);
    ...
    while(!leaderPath.empty() && cmd_status==true)
    {
        if(uav.getUavID() == leaderid)
        {
            ...
            recordSwarmCenter<<"swarmCenter is :"<<swarmCenter.x<<swarmCenter.y<<swarmCenter.z<<"uav 1"<<swarmlocation[1].x<<swarmlocation[1].y<<swarmlocation[1].z<<
            "uav 2"<<swarmlocation[2].x<<swarmlocation[2].y<<swarmlocation[2].z<<"uav 3"<<swarmlocation[3].z<<swarmlocation[3].z<<endl;
                ...
                */
                {
                    ...
                    ros::Duration(8).sleep();//静态队形变换中的等待时间

                }

                onestep(uav,targetPoint,recordFormation3);
                swarmCenter = calculateSwarCenter(uav);
                formationPoints = calFormationPoint(carids,uav.getPosition(),formationnumber);
            ...
            else//队形保持行为
            {
                ...
                //     }
                // }
                onestep(uav,swarmCenter,recordFormation3);
        ...
        }else//从机跟随飞行
        {
            ...
            onestep(uav,tempformationPoint,recordFormation3);
        }
    }
    uav.moveTowardDir(here,moveCtrlMode);
