...
public:
  ...
  simulation_m100::Position start_pos_location;
...
void takeoff_cmd_callback(const simulation_m100::PubSubUavMsg cmd_str);//起飞
void land_cmd_callback(const simulation_m100::PubSubUavMsg cmd_str);//降落
void gohome_cmd_callback(const simulation_m100::PubSubUavMsg cmd_str);//返航
void move_cmd_callback(const simulation_m100::PubSubUavMsg cmd_str);//订阅移动控制指令
void waypoint_cmd_callback(const simulation_m100::PubSubUavMsg cmd_str);//订阅航点控制指令

void attitude_callback(geometry_msgs::Vector3 attit);
void gps_callback(const sensor_msgs::NavSatFix msg);
void position_callback(const simulation_m100::Position pos);
...
bool M100monitoredAction(uint8_t task);
