...
void *context;
string other_uav_address_config("m100_v2/configure/other_uav_address_config_");
...
ros::Publisher disperse_cmd_pub;
ros::Publisher aggregate_cmd_pub;
ros::Publisher start_cmd_pub;
ros::Publisher end_cmd_pub;
ros::Publisher gimbal_target_pub;
...
{ 
    ...
    if(message.msghead().stx() == 254)
    {
        ...
        string log_file(getDirectory() + "m100_v2/log/"+uav_name+"_cmd_receive_log.txt");
        fstream cmd_log;
        cmd_log.open(log_file.c_str(),ios::app);

        cmd_log << "tgt_uav_count:" << message.msghead().tgt_uav_count() << "     ";
        ...
        {   
            ...
            {
                cmd_log << "uav_id:" << message.msghead().tgt_uav_id()[i] << ";";
        ...
        cmd_log << "     " << "msg_type:" << message.msghead().msg_type() << "     ";
        cmd_log << "msg_id:" << message.msghead().msg_id() << "     ";
        cmd_log << "topic_id" << message.msghead().topic_id() << "     ";
        ...
        if(ifToMe == 1)
        {
            ...
            if(message.msghead().msg_type() == 1)//指令消息
            {
                ...
                if(message.msghead().msg_id() == 7)//航点控制指令
                {
                    if(message.msghead().topic_id() == 9)//航点任务
                    {
                        ...
                        m100_v2::UavBaseInfo uav_base_info;
                        ...
                        if(waypoint.kind() == 4)//sync fly（实验用）
                        {
                        ...
                        if(waypoint.kind() == 6)//disperse,分散任务
                        {
                            ROS_INFO("disperse cmd from gcs.");

                            pthread_mutex_lock(&msg_mutex);

							playload_msg.tgt_uav_count = message.msghead().tgt_uav_count();                        
                            playload_msg.start_time = ros::Time::now().toSec();
                            disperse_cmd_pub.publish(playload_msg);                  
                            pthread_mutex_unlock(&msg_mutex);
                        }

                        if(waypoint.kind() == 7)//aggregate，聚集任务
                        {
                            ROS_INFO("aggregate cmd from gcs.");

                            pthread_mutex_lock(&msg_mutex);
                            
							playload_msg.tgt_uav_count = message.msghead().tgt_uav_count();   
                            playload_msg.start_time = ros::Time::now().toSec();
                            aggregate_cmd_pub.publish(playload_msg);                  
                            pthread_mutex_unlock(&msg_mutex);
                        }

						if(waypoint.kind() == 8)//start，开始任务
                        {
                            ROS_INFO("start cmd from gcs.");

                            pthread_mutex_lock(&msg_mutex);
                           
                            playload_msg.start_time = ros::Time::now().toSec();
                            start_cmd_pub.publish(playload_msg);                  
                            pthread_mutex_unlock(&msg_mutex);
                        }
						
						if(waypoint.kind() == 9)//end，结束任务
                        {
                            ROS_INFO("end cmd from gcs.");

                            pthread_mutex_lock(&msg_mutex);
                       
                            playload_msg.start_time = ros::Time::now().toSec();
                            end_cmd_pub.publish(playload_msg);                  
                            pthread_mutex_unlock(&msg_mutex);
                        }
                        
                        cmd_log << "kind:" << waypoint.kind()  << "     " << "waypoints:";
                        myTaskMessage::TaskMessage target_waypoints;
                        target_waypoints.ParseFromString(playload_msg.playload);
                        for (int i=0;i<target_waypoints.point_size(); i++)
                        {
                            cmd_log << target_waypoints.point(i).x() << ";"
                                    << target_waypoints.point(i).y() << ";"
                                    << target_waypoints.point(i).z() << ";"
                                    << "     " ;
                        }
                    }
                }
            ...
            if(message.msghead().msg_type() == 4)//数据消息
            {
                ...
                if(message.msghead().msg_id() == 7)//航点反馈数据：群体飞行时的航点到达状态
                {
                    ...
                    {
                        ROS_INFO("gimbal target update from ground.");

                        pthread_mutex_lock(&msg_mutex);

                        playload_msg.src_uav_id = message.msghead().src_uav_id();
                        //swarm_waypoint_pub.publish(playload_msg); 
                        gimbal_target_pub.publish(playload_msg);
...
int main(int argc, char *argv[])
{
    ...
    takeoff_cmd_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/takeoff_cmd").c_str(),10);
    land_cmd_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/land_cmd").c_str(),10);
    gohome_cmd_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/gohome_cmd").c_str(),10);
    ...
    pathplan_cmd_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/pathplan_cmd").c_str(),10);
    swarm_waypoint_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/swarm_waypoint").c_str(),10);
    formation_cmd_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/formation_cmd").c_str(),10);
    syncfly_task_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/syncfly_task").c_str(),10);
	disperse_cmd_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/disperse_cmd").c_str(),10);
    aggregate_cmd_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/aggregate_cmd").c_str(),10);
	start_cmd_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/start_cmd").c_str(),10);
    end_cmd_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/end_cmd").c_str(),10);

    base_info_pub = nh.advertise<m100_v2::UavBaseInfo>(string(uav_name + "/uav_base_info").c_str(),10);
    recv_info_pub = nh.advertise<m100_v2::CommInfo>(string(uav_name + "/comm_recv_info").c_str(),10);

    other_uav_pos_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/other_uav_pos").c_str(),10);
    other_uav_fd_data_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/other_uav_feedback_data").c_str(),10);
    other_uav_fd_wp_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/other_uav_fd_waypoint").c_str(),10);
    other_uav_mb_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/other_uav_mapbuffer").c_str(),10);
    other_uav_fm_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/other_uav_formation_info").c_str(),10);
    gimbal_target_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uav_name + "/gimbal_target_info").c_str(),10);
