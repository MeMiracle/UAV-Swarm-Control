...
void subSwarmWaypointCallback(m100_v2::PubSubUavMsg target_msg)//实时更新目标航点到达状态
{
...
void* Cooperative_search(void* args)
{
	...
	while (ros::ok())
	{
		...
		char tarFileName[] = "m100_v2/log/recordTarget";
		string tar_file(getDirectory() + tarFileName + uavName.c_str() + ".txt");

		while (true == cmd_status && uav.getPosition().z <= 0.5)
		{
			sleep(0.1);
		}
...
int main(int argc, char** argv)
{
	...
	mapbuffer_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uavName + "/my_mapbuffer").c_str(),10);//局部信息素图通告
	...
	string info_file=getDirectory() + "m100_v2/log/recordROSInfo.txt";
	recordROSInfo.open(info_file.c_str(),ios::app);
	char localmapFileName[] = "m100_v2/log/recordLocalMap";
