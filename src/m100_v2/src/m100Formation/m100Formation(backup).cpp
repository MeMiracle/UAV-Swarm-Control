...
#define MAPLENGTH 20
#define DIAMETER 0.6
#define PATHNUMBER 80 //通过三次B样条插补出来的路径点的个数
#define Formation_expansion_factor 2
...
const float Unmanned_system_diameter = 2;
...
formationType calFormationPoint(vector<int> ids,Point3D formationCenter,int formationnumber =0)
/*
该方法只会由队长无人机所调用
...
//         case 1:
//         {
//             if(swarmlocation.find(carid)!=swarmlocation.end())
//             {
//                 x +=swarmlocation[carid].x;
//                 y +=swarmlocation[carid].y;
//                 z +=swarmlocation[carid].z;
//                 num++;
//             }
//             if(swarmlocation.find(other_uav_id[0])!=swarmlocation.end())
//             {
//                 x +=swarmlocation[other_uav_id[0]].x;
//                 y +=swarmlocation[other_uav_id[0]].y;
//                 z +=swarmlocation[other_uav_id[0]].z;
//                 num++;
//             }
//             if(swarmlocation.find(other_uav_id[1])!=swarmlocation.end())
//             {
//                 x +=swarmlocation[other_uav_id[1]].x;
//                 y +=swarmlocation[other_uav_id[1]].y;
//                 z +=swarmlocation[other_uav_id[1]].z;
//                 num++;
//             }
//             res.x += x;
//             res.y += y;
...
//             res.y /= num;
//             res.z /= num;
// //            cout<<"num is "<<num<<":"<<res.x<<res.y<<res.z<<endl;
//             return res;
//             break;
//         }
//         case 2:
...
void onestep(uav_zyl& theuav,Point3D movePoint,fstream& recordcar,fstream& recordinput)
{
    struct timeval tv;
    struct timezone tz;
    ...
    theuav.moveTowardDir(moveDir,moveCtrlMode,controlconfficient);
    gettimeofday(&tv, &tz);
 ...
 */
{
    ...
    switch(other_uav_count)
    {
        case 0:
        {
            while(!(*thisflag))
            {
                if(isApproimate(uav.getPosition(),myFormationTarget.point,Unmanned_system_diameter*0.2))
                {
                    usleep(10000);
    ...
    while(!leaderPath.empty() && cmd_status==true)
    {
        if(uav.getUavID() == leaderid)
        {
            ...
            //     default:
            //     {
            //         ROS_INFO("uav number error!");
            //         break;
            //     }
            // }
            // ROS_INFO("the second time swarmCenter is %f , %f ",swarmCenter.x,swarmCenter.y);//just for test
                ...
                */
                {
                    ...
                    formationnumber = 1;
                    swarmCenter = calculateSwarCenter(uav);
                    formationPoints = calFormationPoint(carids,uav.getPosition(),formationnumber);
        ...
        }else//从机跟随飞行
        {
            ...
            onestep(uav,tempformationPoint,recordFormation3,recordControlInput);
