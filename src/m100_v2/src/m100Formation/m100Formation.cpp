...
bool sendformationMsg(int uavID,bool formationflag)
{
    ...
    m100_v2::PubSubUavMsg FM_msg_pub;
 ...
 */
{
    ...
    // string fm_file1(getDirectory() + "m100_v2/log/beforeFormation" + uavName.c_str() + ".txt");
    // string fm_file2(getDirectory() + "m100_v2/log/cometoFormation" + uavName.c_str() + ".txt");
    string fm_file3(getDirectory() + "m100_v2/log/Formationrecord" + uavName.c_str() + ".txt");
    string fm_file4(getDirectory() + "m100_v2/log/FormationCenter" + uavName.c_str() + ".txt");
    string fm_file5(getDirectory() + "m100_v2/log/ControlInput" + uavName.c_str() + ".txt");
    string fm_file6(getDirectory() + "m100_v2/log/FormationPoint" + uavName.c_str() + ".txt");
    string fm_file8(getDirectory() + "m100_v2/log/LeaderPath" + uavName.c_str() + ".txt");
...
void *experiment(void* args)
{
    ...
    {   
        ...
        string fm_file7(getDirectory() + "m100_v2/log/FormationPath" + uavName.c_str() + ".txt");
...
*/
void subOtherUavPosCallback(m100_v2::PubSubUavMsg other_uav_pos_msg)//实时更新其他无人机位置
{
...
*/
void subWaypointFromGcsCallback(const m100_v2::PubSubUavMsg waypoint_cmd_msg)//订阅并分析地面站发来的航点序列
{
...
*其中，x y z为目标点位置，yaw为到底目标点后的偏航角，type为目标点类型（局部航点、全局航点等），mark为应该到达该点的无人机的ID，如果为0，则为编队后的运动轨迹点（所有无人机均需到达）
*/
void disperseCmdFromGcsCallback(const m100_v2::PubSubUavMsg disperse_cmd_msg)//订阅并分析地面站发来的航点序列
{
    myTaskMessage::TaskMessage disperse_cmd;//航点序列
    disperse_cmd.Clear();
    disperse_cmd.ParseFromString(disperse_cmd_msg.playload);

    if (disperse_cmd_msg.src_uav_id == 0)//初始化目标点（初次从地面接收到目标点）
	{
        ROS_INFO("received disperse cmd!");

        for(int i=0;i<disperse_cmd.point_size();i++)
        {
            ROS_INFO("%f,%f,%f,%d",disperse_cmd.point(i).x(),disperse_cmd.point(i).y(),disperse_cmd.point(i).z(),disperse_cmd.point(i).mark());
            if(disperse_cmd.point(i).mark() == uav.getUavID())
            {
                myFormationTarget.point.x = disperse_cmd.point(i).x();
                myFormationTarget.point.y = disperse_cmd.point(i).y();
                myFormationTarget.point.z = disperse_cmd.point(i).z();
                // myFormationTarget.markerID = disperse_cmd.point(i).mark();
            }

            if(disperse_cmd.point(i).mark() == 0)
            {
                // Target target(disperse_cmd.point(i).x(),disperse_cmd.point(i).y(),disperse_cmd.point(i).z(),disperse_cmd.point(i).mark());
                // thePathTargets.push_back(target);
            }
        }
...
*其中，x y z为目标点位置，yaw为到底目标点后的偏航角，type为目标点类型（局部航点、全局航点等），mark为应该到达该点的无人机的ID，如果为0，则为编队后的运动轨迹点（所有无人机均需到达）
*/
void aggregateCmdFromGcsCallback(const m100_v2::PubSubUavMsg aggregate_cmd_msg)//订阅并分析地面站发来的航点序列
{
    myTaskMessage::TaskMessage aggregate_cmd;//航点序列
    aggregate_cmd.Clear();
    aggregate_cmd.ParseFromString(aggregate_cmd_msg.playload);

    // thePathTargets.clear();

    if (aggregate_cmd_msg.src_uav_id == 0)//初始化目标点（初次从地面接收到目标点）
	{
        ROS_INFO("received aggregate cmd!");
 //       Target myFormationTarget;//编队形成时的目标点（通常只有一个，用于本机参与形成队形），已经定义为全局变量或其他
 //       vector<Target> thePathTargets;//编队形成后的运动路径目标点序列（一个或多个），已经定义为全局变量或其他
        for(int i=0;i<aggregate_cmd.point_size();i++)
        {
            ROS_INFO("%f,%f,%f,%d",aggregate_cmd.point(i).x(),aggregate_cmd.point(i).y(),aggregate_cmd.point(i).z(),aggregate_cmd.point(i).mark());
            if(aggregate_cmd.point(i).mark() == uav.getUavID())
            {
                myFormationTarget.point.x = aggregate_cmd.point(i).x();
                myFormationTarget.point.y = aggregate_cmd.point(i).y();
                myFormationTarget.point.z = aggregate_cmd.point(i).z();
                //myFormationTarget.markerID = aggregate_cmd.point(i).mark();
            }

            if(aggregate_cmd.point(i).mark() == 0)
            {
                Target target(aggregate_cmd.point(i).x(),aggregate_cmd.point(i).y(),aggregate_cmd.point(i).z(),aggregate_cmd.point(i).mark());
                thePathTargets.push_back(target);
            }
        }
...
*/
void subOtherUavFdWPCallback(m100_v2::PubSubUavMsg other_uav_fd_wp_msg)//实时更新目标航点到达状态（从其他无人机接收到目标点）
{
...
*/
void subOtherFmInfoCallback(m100_v2::PubSubUavMsg other_uav_fm_info)
{
...
int main(int argc,char * argv[])
{
    ...
    ros::Subscriber aggregate_cmd_from_gcs_sub = nh.subscribe(string(uavName + "/aggregate_cmd").c_str(),10,&aggregateCmdFromGcsCallback);
    ros::Subscriber disperse_cmd_from_gcs_sub = nh.subscribe(string(uavName + "/disperse_cmd").c_str(),10,&disperseCmdFromGcsCallback);
    ...
    formation_info_pub = nh.advertise<m100_v2::PubSubUavMsg>(string(uavName + "/my_formation_info").c_str(), 10);
