...
void* getUwbPos3D(void* args)
{
	while(ros::ok())
	{
		...
		char raw_uwb_fileName[] = "m100_v2/log/raw_uwb_log.txt";
        ...
        char valid_uwb_fileName[] = "m100_v2/log/valid_uwb_log.txt";
...
void gps_callback(const sensor_msgs::NavSatFix::ConstPtr& msg)
{
	...
	if(isHomeInited == false)
	{
		...
		{   
		    //如果没有手动设置home点，则在开机初始化时获取当前位置作为home的位置
		    home_gps = current_gps;
		}
		else
		{
			//fix the altitude to make the taking-off plane as the zero plane
    ...
    // {
        if(gps_str)
        {
            gps_str << ros::Time::now() << " " << fixed<<setprecision(6)<<current_gps.longitude << " " << fixed<<setprecision(6)<<current_gps.latitude << " " << current_gps.altitude << endl;      
        }
...
int main(int argc, char *argv[])
{
    ...
    local_pos_pub = nh.advertise<m100_v2::Position>(string(uav_name + "/local_position").c_str(),10);
    global_pos_pub = nh.advertise<m100_v2::Position>(string(uav_name + "/global_position").c_str(),10);

    char gps_log[] = "m100_v2/log/gps_log.txt";
    char dist_log[] = "m100_v2/log/dist_log.txt";
